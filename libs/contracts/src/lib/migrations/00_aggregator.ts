...
import { AggregatorFactoryCode, AggregatorFactoryContractAbstraction, AggregatorFactoryStorage } from '../aggregatorFactory';
...
const aggregatorAddress = storage.values().next().value;

console.log(
  `Aggregator creation done at: ${aggregatorAddress}`
);
...
if (saveToEnv) {
  ...
  return {
    [AGGREGATOR_SMART_CONTRACT_ADDRESS]: aggregatorAddress,
