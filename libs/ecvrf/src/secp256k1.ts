...
const ECVRF_verify = (Y, pi_string, alpha_string) => {
  ...
  return c.eq(c_prime) ? ['VALID', Hex(ECVRF_proof_to_hash(pi_string))] : [null, null];
};
...
const ECVRF_hash_to_curve_try_and_increment = (Y, alpha_string) => {
  ...
  const zero_string = [0x00];
  let H: curve.base.BasePoint | null = null; // null is INVALID
  /**
   ...
   */
  while ((H === null || H?.isInfinity() || !is_on_curve(H)) && ctr < 256) {
...
const string_to_point = (s): curve.base.BasePoint | null => {
  try {
...
const ECVRF_decode_proof = (pi): [curve.base.BasePoint, any, any] | null => {
...
const getFastVerifyComponents = (Y, pi_string, alpha_string) => {
  ...
  return {
    uX: U.x.toString(),
    uY: U.y.toString(),
    sHX: sH.getX().toString(),
    sHY: sH.getX().toString(),
    cGX: cG.getY().toString(),
    cGY: cG.getY().toString()
  };
};
