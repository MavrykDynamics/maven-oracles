...
import {
  AggregatorStorage,
  OraclePriceResponsesForPackValue,
  OraclePriceResponsesValue
} from './aggregators.js';
...
export class ContractService implements OnModuleInit {
  ...
  constructor(private readonly httpService: HttpService, private readonly config: ContractConfig) {
  ...
  public async runVerify() {
    ...
    const oracle1_signature = await this.signOracleObservation(
      oracle1_price,
      oracle1_address,
      oracle1_signer
    );
    const oracle2_signature = await this.signOracleObservation(
      oracle2_price,
      oracle2_address,
      oracle2_signer
    );
    ...
    const oracle1_signature_observations = await this.signOraclePriceResponses(
      oraclePriceResponses_forPack,
      oracle1_signer
    );
    const oracle2_signature_observations = await this.signOraclePriceResponses(
      oraclePriceResponses_forPack,
      oracle2_signer
    );
  ...
  public async signOracleObservation(
    price: BigNumber,
    address: string,
    ...
    const data: MichelsonData = {
      prim: 'Pair',
      args: [{ int: price.toString() }, { string: address }]
    };
  ...
  public async signOraclePriceResponses(
    oraclePriceResponses_forPack: any,
    signer: InMemorySigner
  ): Promise<string> {
    const type2_map: MichelsonType = {
          ...
          args: [
            {
              prim: 'pair',
              args: [
                { prim: 'nat', annots: ['%oracleObservation_price'] },
                { prim: 'address', annots: ['%oracleObservation_address'] }
              ]
            },
      ...
      annots: ['%oraclePriceResponses_forPack']
    };
