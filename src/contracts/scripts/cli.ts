...
import { compile, compileLambdas, runMigrations, compileParameters } from './helpers'
...
yargs
  .command(
    ...
    async (argv) => {
      compile(argv.format, argv.contract, argv.contracts_dir, argv.output_dir, argv.ligo_version, argv.is_apple_silicon)
    },
  )
  .command(
